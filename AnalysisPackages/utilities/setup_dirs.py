...
def main(file_name):
    ...
    for band in range(bands):
        Path(str(Path.cwd().parent.parent) + f"/OutputData/{file_name}/DynamicSpectrum/ch0{str(band)}/") \
            .mkdir(parents=True, exist_ok=True)
        Path(str(Path.cwd().parent.parent) + f"/OutputData/{file_name}/AveragePulseProfile/ch0{str(band)}/") \
            .mkdir(parents=True, exist_ok=True)
        Path(str(Path.cwd().parent.parent) + f"/OutputData/{file_name}/TimeSeries/ch0{str(band)}/") \
            .mkdir(parents=True, exist_ok=True)
